...
public class GamePanel extends Pane {
    ...
    private static final double MIST_BASE_ALPHA = 0.54;
    private static final double MIST_SWIRL_ALPHA = 0.2;
    private static final double MIST_DRIFT_SPEED = 0.2;
    private static final double MIST_COLOR_R = 0.8;
    private static final double MIST_COLOR_G = 0.87;
    private static final double MIST_COLOR_B = 0.89;
    ...
    private void drawMist(GraphicsContext gc, Grid grid, Player player) {
        ...
        for (int y = 0; y < height; y += MIST_SAMPLE_STEP) {
            for (int x = 0; x < width; x += MIST_SAMPLE_STEP) {
                ...
                double swirl = (drift1 * 0.6 + drift2 * 0.4) * MIST_SWIRL_ALPHA;

                double alpha = clamp(distanceOpacity * (MIST_BASE_ALPHA + swirl), 0.0, 0.78);
                if (alpha > 0.01) {
